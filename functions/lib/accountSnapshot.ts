...
const toRows = (
  output: KisInquireBalanceResponse["output2"] | KisInquireBalanceResponse["output1"],
): KisRow[] => {
...
const toHolding = (row: KisRow): AccountHoldingSnapshot | null => {
...
export const fetchAccountSnapshot = async (
  env: Env,
  metrics?: RequestMetrics,
): Promise<AccountSnapshotPayload> => {
  const config = getAccountConfig(env);
  if (!config) {
    throw new Error("계좌 조회를 위해 KIS_ACCOUNT_NO(8자리) 환경변수가 필요합니다.");
  }
  ...
  const holdings = holdingsRows
    .map(toHolding)
    .filter((item): item is AccountHoldingSnapshot => item !== null);

  const holdingsEvalTotal = holdings.reduce((sum, item) => sum + (item.evaluationAmount ?? 0), 0);
  const holdingsPurchaseTotal = holdings.reduce((sum, item) => sum + (item.purchaseAmount ?? 0), 0);
  const holdingsProfitTotal = holdings.reduce((sum, item) => sum + (item.profitAmount ?? 0), 0);
  ...
  const weightBase =
    totalEvaluationAmount && totalEvaluationAmount > 0 ? totalEvaluationAmount : holdingsEvalTotal;
