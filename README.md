...
- 파라미터:
  ...
  - `strategy`: `ALL|VOLUME|HS|IHS|VCP` (기본 `ALL`)
...
- 응답:
  ...
  - `progress`: `{processed,total,remaining,processedCount,ohlcvFailures,insufficientData,...}` 또는 `null`
  - `snapshot`: 마지막 성공 스냅샷 요약 또는 `null`
...
- `.github/workflows/screener-rebuild.yml`
  ...
  - `/api/admin/rebuild-screener`를 batch 호출(기본 20)하며 `inProgress=false`까지 반복
