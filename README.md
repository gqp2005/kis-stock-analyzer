...
- 보조 API: `/api/account-diagnostics` (보유종목 진단)
- 보조 API: `/api/autotrade` (50만원 계좌 자동매매 전략 후보/실행/포지션)
- 보조 API: `/api/trade/candidates` (오늘의 자동매매 후보 카드 조회)
- 보조 API: `/api/trade/order` (KIS 주문 상태 머신 실행)
- 보조 API: `/api/dashboard` (관심종목 알림/시장 온도계/전략 랭킹/타임라인)
- 보조 API: `/api/favorites` (owner 관심종목 서버 저장/동기화)
...
- 프론트: 상단 탭으로 `종목 분석` / `종목 추천(스크리너)` / `전략` / `자동매매` / `내 계좌` / `운영(관리자)` 전환
- 프론트: 상단 탭으로 `운영(관리자)` 패널 제공(상태/실패/히스토리/수동 실행)
- 프론트: 관심종목(즐겨찾기) 저장 + 관심 알림 패널 + 브라우저 알림(옵션)
- 프론트: 관심종목은 owner 서버 저장을 우선 사용하고, 서버 저장소 미설정 시 브라우저 localStorage로 fallback
- 프론트: 스크리너에 `거래대금 상위 500 유니버스`/마지막 갱신 시각 표시
- 프론트: 시장 전체 온도계 / 전략별 성과 랭킹 / 전략 후보 타임라인 표시
...
- `SCREENER_KV`/`SCREENER_DB`는 문자열 env가 아니라 Cloudflare Pages의 Binding으로 연결합니다.
  ...
  - owner 관심종목(`/api/favorites`)도 같은 영속 저장소(`SCREENER_KV` 또는 `SCREENER_DB`)를 사용합니다.
...
5. (선택) Bindings:
   ...
   - KV Namespace 바인딩명: `AUTOTRADE_KV` (자동매매 포지션/손익/쿨다운 저장)
...
- owner 관심종목 서버 저장/조회 API
- 인증:
  - 브라우저 로그인 세션(`SITE_AUTH_PASSWORD`) 또는 `ADMIN_TOKEN`
- 저장소:
  - `SCREENER_KV` 또는 `SCREENER_DB`
  - 둘 다 없으면 프론트는 브라우저 `localStorage` fallback 사용
- 동작:
  - `GET`: 현재 owner 관심종목 목록 조회
  - `POST`: 단일 종목 추가 `{ code, name }`
  - `DELETE`: 단일 종목 삭제 `?code=005930`
  - `PUT`: 전체 목록 치환 `{ items: [...] }`
...
### `GET /api/autotrade?market=ALL&universe=200&dryRun=1`
### `POST /api/autotrade`
- 50만원 계좌용 자동매매 전략 v1 실행/조회
- 전략 핵심:
  - 유니버스: 거래대금 상위 200~500(스크리너 스냅샷 기반)
  - 진입: `close>MA60`, `MA20>MA60`, 거래대금 재유입 흔적, 최근 3~10봉 조정 + 거래량 감소, A/B/C 트리거
  - 손절: `invalidLow` 또는 최근 스윙 저점 기준(단 최대 5%)
  - 익절: 1R 50% / 2R 잔여 전량 / 10거래일 1R 미도달 시 시간청산
  - 자금관리: 1회 손실 5,000원, 1일 손실 10,000원, 종목당 150,000원, 동시보유 2종목
- 실행 모드:
  - `GET`: 기본 조회(`execute=false`)
...
### `GET /api/trade/candidates?market=ALL&universe=200`
- 오늘의 자동매매 후보 카드 데이터 조회
- 응답:
  - `summary`: 일일 손실/신규진입 차단 여부/오픈 포지션 수
  - `candidates[]`: `entry/stop/tp1/tp2/qty/maxLossWon/riskPct/reasons/warnings`
- 프론트 `오늘의 자동매매 후보 카드` 섹션은 이 엔드포인트를 사용

### `POST /api/trade/order`
- 후보 카드에서 단일 종목 주문 상태 머신 실행
- 요청 body:
  - `code` (필수)
  - `market`, `universe`
  - `adminToken` 또는 `x-admin-token` (권장)
  - `autoExecute` (UI 옵션)
  - `useHashKey` (옵션, POST body hashkey 헤더)
  - `retryOnce` (옵션)
  - `dryRun` (옵션)
  - `clientOrderId` (옵션, 미지정 시 서버 생성)
- 상태 머신:
  - `IDLE → PRECHECK → ORDER_SUBMITTING → ORDER_ACCEPTED → WORKING → PARTIALLY_FILLED/FILLED → POSITION_OPEN`
  - 취소 흐름: `WORKING → CANCEL_REQUESTED → CANCELED`
  - 실패 흐름: `ORDER_REJECTED`
- 동작:
  - PRECHECK: 일일/1회 손실, 1종목 투입, 동시보유, `qty<1` 검증
  - WORKING: REST 폴링(체결 조회) 후 45초 내 미체결 잔량 취소
  - idempotency: `clientOrderId`를 KV에 저장해 중복 주문 방지
  - 로그: 상태 전이별 시각/사유/요약 저장(토큰 값 비저장)
